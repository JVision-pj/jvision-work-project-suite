...
const stages: Stage[] = ["待辦", "進行中", "審核", "完成"];
const priorities: Priority[] = ["高", "中", "低"];
const owners = ["Mia", "Leo", "Nina", "Ryan"];

export function WorkProjectDemo() {
  const [tasks, setTasks] = useState<Task[]>([
    { id: 1, title: "產品發布需求盤點", project: "CRM 專案", owner: "Mia", due: "7/12", hours: 8, stage: "進行中", priority: "高" },
    { id: 2, title: "首頁新版線框設計", project: "網站改版", owner: "Leo", due: "7/15", hours: 6, stage: "待辦", priority: "中" },
    { id: 3, title: "AI 狀態摘要規則", project: "營運中台", owner: "Nina", due: "7/18", hours: 4, stage: "審核", priority: "高" },
    { id: 4, title: "上線 QA 檢查表", project: "系統上線", owner: "Ryan", due: "7/19", hours: 3, stage: "完成", priority: "低" },
  ]);
  ...
  const metrics = useMemo(() => {
    const pending = tasks.filter((task) => task.stage !== "完成").length;
    const highRisk = tasks.filter((task) => task.priority === "高" && task.stage !== "完成").length;
    const totalHours = tasks.reduce((sum, task) => sum + task.hours, 0);
    const byOwner = owners.map((owner) => ({
      owner,
      hours: tasks.filter((task) => task.owner === owner && task.stage !== "完成").reduce((sum, task) => sum + task.hours, 0),
    }));
  ...
  const aiSummary = `目前共有 ${tasks.length} 個任務，${metrics.pending} 個尚未完成。高優先級項目 ${metrics.highRisk} 個，建議先處理「${
    tasks.find((task) => task.priority === "高" && task.stage !== "完成")?.title ?? "無"
  ...
  function addTask(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage(`${title} 已加入待辦，Jvision 會同步更新看板與工作負荷。`);
  ...
  function moveTask(id: number, direction: -1 | 1) {
    ...
    setMessage("看板狀態已更新，系統會重新整理專案進度與 AI 摘要。");
  }
              ...
              <div className="stage" key={stage}>
                ...
                {tasks.filter((task) => task.stage === stage).map((task) => (
                  <article className="task-card" key={task.id}>
                    ...
                    <small>{task.owner} · {task.due} · {task.hours}h · {task.priority}</small>
                    <div>
                      <button type="button" onClick={() => moveTask(task.id, -1)}>←</button>
                      <button type="button" onClick={() => moveTask(task.id, 1)}>→</button>
                    </div>
                  </article>
                ))}
